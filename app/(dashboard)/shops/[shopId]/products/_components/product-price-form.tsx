...
export default function PriceForm({ initialData, productId, shopId }: ProductPriceFormProps) {
  ...
  const form = useForm<ProductFormValues>({
    ...
    defaultValues: {
      ...
      currency: (initialData?.currency as "IDR" | "USD" | "EUR") || "IDR",
    },
  });
  ...
  return (
    <div className="mt-6 border bg-slate-100 dark:bg-slate-900 rounded-md p-4">
      ...
      {!isEditing ? (
        <p className={`text-sm mt-2 ${!initialData.price ? "text-slate-500 italic" : ""}`}>{initialData.price ? formatPrice(initialData.price, (initialData.currency as "IDR" | "USD" | "EUR") || "IDR") : "No Price Set"}</p>
      ) : (
        <Form {...form}>

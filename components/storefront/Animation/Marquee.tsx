...
const messages = [
  {
    text: "Join our ecommerce student community to learn digital marketing and entrepreneurship",
  },
  {
    text: "Connect with fellow students building their online businesses",
  },
];
...
const TextMessage = ({ text }: { text: string }) => {
  return (
    <div
      className={cn(
        "relative mx-4 cursor-pointer rounded-xl border px-8 py-4",
        // light styles
        "border-gray-950/[.1] bg-gray-950/[.01] hover:bg-gray-950/[.05]",
        // dark styles
        "dark:border-gray-50/[.1] dark:bg-gray-50/[.10] dark:hover:bg-gray-50/[.15]"
...
export function MarqueeDemo() {
  return (
    <div className="relative flex h-24 w-full flex-col items-center justify-center overflow-hidden border bg-background">
      <Marquee pauseOnHover className="[--duration:30s]">
        {[...messages, ...messages, ...messages].map((message, idx) => (
          <TextMessage key={idx} {...message} />
        ))}
      </Marquee>
